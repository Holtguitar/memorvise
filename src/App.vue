<script setup>
</script>

<template>
  <nav>
    <router-link id="logo" to="/" v-if="isLoggedIn">Memorvise</router-link>
    <div id="logo-else" to="/" v-else="isLoggedIn">Memorvise</div>
    <router-link class="nav-item" id="home" to="/"  v-if="isLoggedIn">Account Info</router-link>
    <router-link class="nav-item" id="cards" to="/cards" v-if="isLoggedIn">Cards</router-link>
    <router-link class="nav-item" id="card-creator" to="/card-creator" v-if="isLoggedIn">Create</router-link>
    <router-link  id="sign-in" to="/sign-in" v-if="!isLoggedIn">Sign In</router-link>
    <button @click="handleSignOut"  id="sign-out" v-if="isLoggedIn">Sign Out</button>
  </nav>
  <router-view/>
</template>

<script setup>
  import {onMounted, ref} from "vue";
  import { getAuth, onAuthStateChanged, signOut } from "firebase/auth";
  import router from "./router"

  const isLoggedIn = ref(false);

  let auth;
  onMounted(() => {
    auth = getAuth();
    onAuthStateChanged(auth, (user) => {
      if(user){
        isLoggedIn.value = true;
      } else {
        isLoggedIn.value = false;
      }
    })
  });

  const handleSignOut = () => {
    signOut(auth).then(() => {
      router.push("/sign-in");
    })
  }
</script>

<style>

nav {
  position: -webkit-sticky;
  display: flex;
  margin-top: -1%;
  justify-content: center;
  text-align: center;
  margin-bottom: 50px !important;
  width: 100vw;
  height: 90px;
  left: -9.2%;
  top: -5%;
  background-color: whitesmoke;
}

.nav-item {
  margin-right: 30px;
  padding: 5px;
  top: 30%;
  font-size: 30px;
  height: 60px;
  font-family: 'Shadows Into Light', cursive;
}

.nav-item:hover {
  text-decoration: underline;
}

#logo {
  position: absolute;
  padding: 5px;
  top: 30%;
  left: 5%;
  font-size: 30px;
  height: 60px;
  font-family: 'Playball', cursive;
  text-decoration: underline;
}

#logo-else {
  position: absolute;
  padding: 5px;
  top: 30%;
  left: 5%;
  font-size: 30px;
  height: 60px;
  font-family: 'Playball', cursive;
  text-decoration: underline;
  color: rgb(16, 122, 87)
}

#sign-out, #sign-in {
  position: absolute;
  margin-right: 30px;
  padding: 5px;
  top: 30%;
  left: 80%;
  font-size: 30px;
  height: 60px;
  border: none;
  background-color: transparent;
  text-decoration: none;
  color: rgb(16, 122, 87);
  font-weight: 600;
  font-family: 'Shadows Into Light', cursive;
  --c: rgb(16, 122, 87); /* the border color */
  --b: 3px;    /* the border thickness*/
  --g: 5px;     /* the gap on hover */
  
  padding: calc(var(--g) + var(--b));
  --_g: #0000 25%,var(--c) 0;
  background: 
    conic-gradient(from 180deg at top    var(--b) right var(--b),var(--_g))
     var(--_i,200%) 0  /200% var(--_i,var(--b))  no-repeat,
    conic-gradient(            at bottom var(--b) left  var(--b),var(--_g))
     0   var(--_i,200%)/var(--_i,var(--b)) 200%  no-repeat;
  transition: .3s, background-position .3s .3s;
  cursor: pointer;
}

#sign-in {
  position: absolute;
  /* margin-left: 80%; */
}

#sign-out:hover, #sign-in:hover {
  --_i: 100%;
  transition: .3s, background-size .3s .3s;
  cursor: pointer;
}

#sign-out {

}
#sign-out:hover {
 
}





</style>